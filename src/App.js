        ...
        <div class="w-96 h-60 border-2 rounded-lg flex justify-center items-center">
          {/* <table class="table-auto">
            <tbody>
              <tr>
                <td>
                  <span class="font-bold bg-clip-text text-transparent bg-gradient-to-r from-pink-600 to-violet-500">
                    Select Your Car:
                  </span>
                </td>
                <td>
          ...
          <div className="flex flex-col gap-y-4">
            <Select id="car" label="Select Your Car" options={options} />
            <Select
              id="year"
              label="Select Your Car Year"
              options={options.map((item) => ({ label: item.year }))}
            />
          </div>
        </div>
      </div>
